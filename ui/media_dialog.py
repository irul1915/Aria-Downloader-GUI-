...
class MediaDownloaderDialog(QDialog):
    def __init__(self, parent=None, prefill_url=None):
        ...
        self._prefill_url = prefill_url      # Simpan untuk auto-analyze setelah showEvent
        
        self.setWindowTitle("Media Extractor (yt-dlp + FFmpeg)")
        self.setMinimumSize(750, 500)
        self.setup_ui()
        
        # Isi URL jika ada prefill
        if prefill_url:
            self.url_input.setText(prefill_url)

    def showEvent(self, event):
        """Override showEvent untuk memicu auto-analyze setelah dialog terlihat."""
        super().showEvent(event)
        if self._prefill_url:
            # Delay 500ms agar UI selesai dirender sebelum analyze (mencegah freeze visual)
            QTimer.singleShot(500, self.start_analysis)
            self._prefill_url = None  # Hanya sekali
