...
class AddDownloadDialog(QDialog):
    def __init__(self, parent=None, prefill_url=None):
        ...
        # --- FITUR BARU: Prefill URL dari Clipboard Monitor ---
        if prefill_url:
            self.url_input.setText(prefill_url)
            # Jika magnet link, langsung fokus ke tombol OK untuk kemudahan (tekan Enter langsung download)
            if prefill_url.startswith('magnet:'):
